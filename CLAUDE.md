...
Following up on that same audit (`20260730032155_add_admin_override_to_scoring_and_roster_rls.sql`) found the actual scoring path had it worse: `can_score_match()` — the single function gating INSERT/UPDATE on `match_events`, `cricket_innings`, `cricket_player_stats`, `golf_holes`, and `golf_scores` — had no admin branch at all (only `is_match_host()` or a `match_players` row with role `player`/`scorer`), so an admin scoring a match they didn't create or join got a hard RLS-violation error on every tap, not a silent no-op. Fixed by adding `OR is_admin_session()` inside `can_score_match()` itself, which covers all five tables in one place. Same class of gap existed on `match_players`/`match_teams` INSERT (host-only, breaking the Edit Roster "Add Player" button for a non-host admin) and UPDATE (no current call site, fixed anyway for consistency). If you add a new table gated by `can_score_match()` or a new admin-only write path, this function is the one place to check first.
...
`player_career_stats` no longer exists — the client stopped writing to it in `20260729224346_revoke_client_writes_player_career_stats.sql` (revoking INSERT/UPDATE), and since nothing read it either, it was dropped outright in `20260730030956_drop_dead_player_career_stats_table.sql` (see Stats pipeline above). If this name resurfaces in an old migration file, a stale doc, or a linter suggestion, it's referring to a table that's gone.
