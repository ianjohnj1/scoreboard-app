...
- `match_event_points.test.sql` — 51 assertions covering all 17 branches of `match_event_points()` (16 recognized event types + the shared `ELSE` fallback, which `pool_miss`/`pool_foul` also exercise since they score 0 with no dedicated `WHEN`), the Postgres function that decides what a `match_event` is worth toward the leaderboard (see Stats pipeline below), plus `darts_event_score()`'s single-throw/darts-array paths and the `point`/`score` `amount || 1` JS-falsy quirk. Pure `SELECT`s against synthetic jsonb — no tables touched at all.
...
`PoolRoom`'s four event types are scored as of `20260801124346_score_pool_ball_tracking_events.sql`: `pool_ball_potted` is 1 point but only when `event_data.own_ball` is `true` (a ball potted into the opponent's group scores nothing for the shooter — see `poolEngine.ts`'s `applyPoolPot`); `pool_miss`/`pool_foul` score 0; `pool_game_won` is a flat 7-point bonus to the winner. 7 isn't arbitrary — a legal win requires potting all 7 of your own group first (7 points before the bonus), and a player who *loses* via the illegal-early-black foul can have at most 6 own-group balls potted (a 7th would have made their black attempt legal, i.e. a win). So the winner's `points` total is always strictly greater than the loser's, independent of `matches_won`/`matches_lost` (those already come from `winner_profile_id`/`winner_team_id` directly, not from `points` — see `getGlobalLeaderboardData()` in `stats.ts`). Still deliberately out of scope: a dedicated balls-potted/fouls/streak counter column in the view, and "win % when Bigs/Smalls" or "win % when broke first" — those need their own column(s) and `GlobalPlayerStats`/UI wiring, not just a points branch. Group assignment isn't a persisted event at all; it's a pure function of which side pots the table-opening ball (`ballGroupOf()` in `poolEngine.ts`), so those two stats would need to replicate that same open-table rule in SQL (or add a dedicated event) rather than assume one exists.
