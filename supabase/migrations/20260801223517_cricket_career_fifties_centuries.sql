-- Surfaces cricket_player_stats.scored_fifty/scored_century as career
-- "50s"/"100s" counts, the same shape as the Pool analytics additions in
-- 20260801211316_pool_career_analytics.sql - the raw signal was already
-- being written per-innings by CricketRoom.tsx's handleDelivery on every
-- milestone, but nothing ever read it back (confirmed via grep before
-- writing this: only CricketRoom.tsx itself touched those two columns).
--
-- A century also flips scored_fifty true in CricketRoom.tsx (`if
-- (!stat?.scored_fifty) batterUpdate.scored_fifty = true;` alongside
-- scored_century), so cricket_fifties is deliberately exclusive of
-- centuries here (scored_fifty AND NOT scored_century) - matches standard
-- cricket scorecard convention, where a century isn't also counted as a
-- fifty, rather than naively summing the raw flag and double-counting.
--
-- Appends two columns to the end of both cricket_metrics and the outer
-- SELECT list so CREATE OR REPLACE VIEW doesn't need to touch any existing
-- column - everything else in this view is byte-for-byte the same as
-- 20260801211316_pool_career_analytics.sql.
...
cricket_metrics AS (
    SELECT
        ...
        SUM(CASE WHEN scored_fifty AND NOT scored_century THEN 1 ELSE 0 END) as cricket_fifties,
        SUM(CASE WHEN scored_century THEN 1 ELSE 0 END) as cricket_centuries
...
SELECT
    ...
    SUM(COALESCE(crm.cricket_fifties, 0))::int as total_cricket_fifties,
    SUM(COALESCE(crm.cricket_centuries, 0))::int as total_cricket_centuries
FROM player_match_status pms
