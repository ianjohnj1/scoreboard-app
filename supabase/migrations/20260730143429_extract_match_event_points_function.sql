-- Follow-up to 20260730143009. That migration inlined the per-event scoring
-- rules as a CASE inside leaderboard_match_player_scores, which meant the only
-- way to exercise them was to have matching rows in match_events. The live
-- database holds three event types (chip_off_score, golf_score, bball_score),
-- so eleven of the fourteen branches had no reachable test at all.
--
-- Extracting the CASE into a function makes every branch callable directly
-- against synthetic jsonb - `SELECT match_event_points('darts_win', '{...}')` -
-- with no rows written anywhere. That's the only practical way to verify the
-- sports nobody has played yet. Behaviour is unchanged; this is a pure
-- refactor of 20260730143009 for testability.
--
-- The function is a single IMMUTABLE SQL expression so Postgres inlines it
-- into the view rather than paying a call per event row.

CREATE OR REPLACE FUNCTION public.match_event_points(
  p_event_type text,
  p_event_data jsonb,
...
event_totals AS (
  SELECT
    r.match_id,
    r.profile_id,
    sum(match_event_points(r.event_type, r.event_data, r.profile_id)) AS points,
