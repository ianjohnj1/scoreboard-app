-- Drop and recreate player_career_analytics view with Darts sub-game metrics
...
darts_atw_metrics AS (
    SELECT
        ...
        match_id,
        COUNT(*) as atw_darts,
        COUNT(DISTINCT CASE WHEN event_data->>'winner_profile_id' = player_id THEN match_id END) as atw_wins
    FROM match_events
...
darts_killer_metrics AS (
    SELECT
        ...
        COUNT(CASE WHEN (event_data->'assignments'->player_id->>'isKiller')::text = 'true' OR (event_data->>'activated')::text = 'true' THEN 1 END) as killer_throws,
