...
export default function MatchRoomPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes',
        { event: '*', schema: 'public', table: 'match_rooms', filter: `id=eq.${match.id}` },
        () => loadMatch()
      )
      .on('postgres_changes',
  ...
  const SportRoom = getSportRoom(match.sport);
...
function getSportRoom(sport: string): React.ComponentType<{ ctx: MatchContext }> {
