...
import {
  ...
  QrCode, Zap, Crown, BarChart2, Trash2
} from 'lucide-react';
import Avatar from '../components/Avatar';
import QRCodeModal from '../components/QRCodeModal';
import { useAuth } from '../contexts/AuthContext';
import { getRecentMatches, getActiveMatches, getLiveActivity, deleteMatch } from '../lib/matches';
        ...
        <section>
          ...
          <div className="space-y-2">
            {liveActivity.map(({ session, profile, match }) => {
              ...
              const sportLabel = (match.sport || 'custom').toUpperCase();
                    ...
                    <div className="min-w-0">
                      <p className="text-charcoal-100 font-semibold text-sm">
                        ...
                        <span className="text-charcoal-200">
                          {match.sport === 'cricket' ? '🏏' : '⛳'} {sportLabel} ({match.room_code || '....'})
                        </span>
                      </p>
                    </div>
                  </div>
              ...
              {activeMatches.map(match => {
                ...
                return (
                  <MatchCard 
                    ...
                    canDelete={match.created_by === currentUser?.id}
    ...
    <div className={`bg-charcoal-800 border border-charcoal-700 rounded-xl p-3 flex items-center gap-3 hover:border-charcoal-600 transition-all duration-150 ${isDeleting ? 'opacity-50 grayscale pointer-events-none' : ''}`}>
      ...
      <div className="flex-1 min-w-0" onClick={handleCardClick} style={{ cursor: isDeleting ? 'default' : 'pointer' }}>
        <div className="flex items-center gap-2">
          ...
          <span className={`px-2 py-0.5 rounded text-xs font-bold ${isLive ? 'bg-emerald-950 text-emerald-400' : 'bg-charcoal-700 text-charcoal-400'}`}>
            {isLive ? 'Live' : 'Done'}
          </span>
        </div>
